...
public class LongRidesUnitTest extends LongRidesTestBase {
	...
	private final KeyedProcessFunction<Long, TaxiRide, Long> javaExercise = new LongRidesExercise.AlertFunction();

	protected ComposedKeyedProcessFunction composedAlertFunction() {
		return new ComposedKeyedProcessFunction<>(javaExercise, javaExercise);
	}
	...
	public void shouldNotAlertWithoutWatermarkOrEndEvent() throws Exception {
		...
		Watermark mark2HoursLater = new Watermark(BEGINNING.plusSeconds(2 * 60 * 60).toEpochMilli());
		harness.processWatermark(mark2HoursLater);

		StreamRecord<Long> rideIdAtTimeOfWatermark = new StreamRecord<>(rideStarted.rideId,
	...
	public void shouldAlertOnWatermark() throws Exception {
		...
		StreamRecord<Long> rideIdAtTimeOfWatermark = new StreamRecord<>(startOfLongRide.rideId,
				mark2HoursLater.getTimestamp());
		assertThat(harness.getOutput()).containsExactly(rideIdAtTimeOfWatermark, mark2HoursLater);
	}
	...
	private Long resultingRideId() {
		...
		assertThat(results.size()).withFailMessage("Expecting test to have exactly one result").isEqualTo(1);
	...
	private KeyedOneInputStreamOperatorTestHarness<Long, TaxiRide, Long> setupHarness(
		...
		KeyedOneInputStreamOperatorTestHarness<Long, TaxiRide, Long> testHarness = new KeyedOneInputStreamOperatorTestHarness<>(
				operator, r -> r.rideId, Types.LONG);
