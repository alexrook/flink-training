 ...
 * start event and a taxi ride end event. The isStart flag specifies the type of
 * the event.
 *
 * <p>
 * A TaxiRide consists of - the rideId of the event which is identical for start
 * and end record - the type of the event (start or end) - the time of the event
 * - the longitude of the start location - the latitude of the start location -
 * the longitude of the end location - the latitude of the end location - the
 * passengerCnt of the ride - the taxiId - the driverId
 */
public class TaxiRide implements Comparable<TaxiRide>, Serializable {
	...
	public TaxiRide(long rideId, boolean isStart, Instant eventTime, float startLon, float startLat, float endLon,
			float endLat, short passengerCnt, long taxiId, long driverId) {
		this.rideId = rideId;
		this.isStart = isStart;
	...
	public String toString() {
		...
		return rideId + "," + (isStart ? "START" : "END") + "," + eventTime.toString() + "," + startLon + "," + startLat
				+ "," + endLon + "," + endLat + "," + passengerCnt + "," + taxiId + "," + driverId;
	}
	...
	public boolean equals(Object o) {
		...
		return rideId == taxiRide.rideId && isStart == taxiRide.isStart
				&& Float.compare(taxiRide.startLon, startLon) == 0 && Float.compare(taxiRide.startLat, startLat) == 0
				&& Float.compare(taxiRide.endLon, endLon) == 0 && Float.compare(taxiRide.endLat, endLat) == 0
				&& passengerCnt == taxiRide.passengerCnt && taxiId == taxiRide.taxiId && driverId == taxiRide.driverId
				&& Objects.equals(eventTime, taxiRide.eventTime);
	}

	@Override
	public int hashCode() {
		return Objects.hash(rideId, isStart, eventTime, startLon, startLat, endLon, endLat, passengerCnt, taxiId,
				driverId);
	}
	...
	/**
	 * Creates a StreamRecord from this taxi ride, using its id and timestamp. Used
	 * in tests.
	 */
	@VisibleForTesting
